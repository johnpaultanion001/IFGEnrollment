            ...
            <div class="card">
                    <div class="card-header p-2">
                        <h4>
                            ...
                            <button type="button" id="create_record" class="btn btn-primary btn-sm">New Referral Code</button>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <table class="table align-items-center table-flush datatable-country display" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                    @foreach($referralCodes as $data)
                                        <tr>
                                            <td>
                                                <button type="button" edit="{{  $data->id ?? '' }}" class="edit btn btn-info btn-sm"><i class="fas fa-pen"></i></button>
                                                <button type="button" remove="{{  $data->id ?? '' }}" class="remove btn btn-danger btn-sm"><i class="fas fa-trash"></i></button>
                                            </td>
                                            <td>
                                                {{  $data->referral_code ?? '' }}
                                            </td>
                                            <td>
                                                <h6 class="card-subtitle mb-3 badge {{  $data->isUsed == true ? 'badge-success' : 'badge-warning' }}">{{  $data->isUsed == true ? 'YES':'NO' }}</h6>
                                            </td>
                                            <td>
                                                {{  $data->remarks ?? '' }}
                                            </td>
                    ...
                    <div id="modalbody" class="modalbody row">
                            <div class="col-sm-12">
                                <div class="form-group">
                                    <label class="control-label text-uppercase" >Referral Code: <span class="text-danger">*</span> </label>
...
$('#myForm').on('submit', function(event){
    ...
    $('.form-control').removeClass('is-invalid')
    var action_url = "{{ route('admin.referral_code.store') }}";
    var type = "POST";

    if($('#action').val() == 'Edit'){
        var id = $('#hidden_id').val();
        action_url = "/admin/referral_code/" + id;
        type = "PUT";
    }
...
$(document).on('click', '.edit', function(){
    ...
    $.ajax({
        url :"/admin/referral_code/"+id+"/edit",
        dataType:"json",
        beforeSend:function(){
            $("#action_button").attr("disabled", true);
            $("#action_button").attr("value", "Loading..");
